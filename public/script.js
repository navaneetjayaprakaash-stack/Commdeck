...
function initSocket() {
  ...
  socket.on("chatMessage", (msg) => {
    // Check for message duplication by verifying UID and timestamp
    if (msg.uid === user.uid && Date.now() - msg.ts > 500) {
        return; // Prevents showing own message echo
    }
...
function sendMessage() {
  ...
  if (currentDm) {
    ...
    addMessage(msg, "dmPanel"); // For DMs, show message immediately
  } else {
    socket.emit("chatMessage", msg); // Let the server echo the message
  }
  input.value = "";
