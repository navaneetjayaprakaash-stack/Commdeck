...
import { getFirestore, doc, collection, addDoc, setDoc, deleteDoc, onSnapshot, query, orderBy, updateDoc, serverTimestamp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
async function joinChatroom(linkOrId) {
  ...
  const participantsRef = doc(db, "chatrooms", currentChatroomId);
  onSnapshot(participantsRef, snap => {
    const data = snap.data();
    participantsList.innerHTML = "";
    for(const [uid] of Object.entries(data.participants || {})) {
      const chip = document.createElement("span");
      chip.className = `participant-chip ${uid===userId ? "you" : ""}`;
      chip.textContent = uid===userId ? "You" : uid.substring(0,8)+"...";
      participantsList.appendChild(chip);
    }
  });

  document.getElementById("chatroom-title").textContent = `Room: ${currentChatroomId.substring(0,8)}...`;
}
