...
// Serve static files from the root directory for client access
...
io.on("connection", (socket) => {
    ...
    // joinRoom accepts and stores the Firebase UID (firebaseId)
    ...
    socket.on("privateMessage", ({ to, text, recipientFirebaseId }) => {
        ...
        if (fromUser && users[to] && socket.id !== to) {
            // Send the private message notification to the recipient's socket
