...
function emitRoomList(){ io.emit("roomList",Array.from(rooms)); }
...
io.on("connection",socket=>{
  ...
  socket.on("joinRoom",async ({room,uid,name})=>{
    ...
    const messagesSnap=await db.collection("rooms").doc(room).collection("messages").orderBy("ts","asc").limit(100).get();
  ...
  socket.on("disconnect",()=>{
    ...
    if(user){
      ...
      delete uidToSocket[user.uid];
      delete users[socket.id];
      io.to(room).emit("chatMessage",{from:"system",text:`${name} left ${room}`,uid:null,ts:Date.now(),system:true});
      emitRoomUsers(room);
    }
